 ...
 * A majority of the inputs from the oracles are required to be the same to update the data. (ex 75%)
 * @dev see {IOracles}
 */
contract Oracles is IOracles, OwnablePausableUpgradeable {
  ...
  //---------// Initialization and nonce getters //---------//
  function initialize(
    address _admin,
    address _rewards,
  ...
  function addOrchestrator(address account) external override onlyAdmin {
  ...
  function removeOrchestrator(address account) external onlyAdmin {
    require(hasRole(ORCHESTRATOR_ROLE, account), "Orchestrators: Account isn't an orchestrator");
  ...
  function removeOracle(address account) external override onlyAdmin {
    require(hasRole(ORACLE_ROLE, account), 'Oracles: the account is not an oracle');
  ...
  function submitRewards(
    ...
    require(_isEnoughSignatures(signatures.length), 'Oracles: invalid number of signatures');
  ...
  function submitMerkleRoot(
    ...
    require(isMerkleRootVoting(), 'Oracles: merkle root voting is not active, too early');
    require(_isEnoughSignatures(signatures.length), 'Oracles: invalid number of signatures');
  ...
  /**
   * @dev See {IOracles-processUnstake}.
   */
   ...
   */
  function _isEnoughSignatures(uint256 signaturesCount) internal view returns (bool) {
