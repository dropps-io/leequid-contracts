...
contract PoolValidators is IPoolValidators, OwnablePausableUpgradeable, ReentrancyGuard {
  ...
  function getOperator(address _operator) external view override returns (bytes32, bool) {
    ...
    return (operator.depositDataMerkleRoot, operator.committed);
  }
  ...
  function addOperator(
    ...
    require(
      ...
      'PoolValidators: same merkle root'
    );
    ...
    emit OperatorAdded(_operator, depositDataMerkleRoot, depositDataMerkleProofs);
  }
  ...
  function commitOperator() external override whenNotPaused {
    ...
    require(
      operator.depositDataMerkleRoot != bytes32(0) && !operator.committed,
  ...
  function removeOperator(address _operator) external override whenNotPaused {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || msg.sender == _operator,
  ...
  function registerValidator(
    ...
    require(
      depositDataMerkleRoot != bytes32(0) && operator.committed,
      'PoolValidators: invalid operator'
    );
