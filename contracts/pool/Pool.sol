...
contract Pool is IPool, OwnablePausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(
    ...
    require(
      _validatorRegistration != address(0),
      'Pool: validatorRegistration address cannot be zero'
    );
   ...
   */
  function setPendingValidatorsLimit(
    uint256 newPendingValidatorsLimit
  ...
  function setActivatedValidators(uint256 newActivatedValidators) external override {
    require(
      msg.sender == oracles || hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
      'Pool: access denied'
    );
  ...
  function receiveWithoutActivation() external payable override {
    require(
      msg.sender == address(stakedLyxToken) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
      'Pool: access denied'
    );
  }
   ...
   */
  function stakeWithPartnerOnBehalf(
    address partner,
    address recipient
   ...
   */
  function stakeWithReferrerOnBehalf(
    address referrer,
    address recipient
  ) external payable override nonReentrant {
    // stake amount
  ...
  function _stake(address recipient, uint256 value) internal whenNotPaused {
    ...
    uint256 _pendingValidators = pendingValidators +
      (address(this).balance / VALIDATOR_TOTAL_DEPOSIT);
    ...
    if (
      validatorIndex * 1e4 <=
      _activatedValidators * 1e4 + effectiveValidators() * pendingValidatorsLimit
    ) {
      stakedLyxToken.mint(recipient, value, true, '');
    } else {
      ...
      activations[recipient][validatorIndex] =
        activations[recipient][validatorIndex] +
        _valueToDeposit;
  ...
  function activateMultiple(
    address account,
    uint256[] calldata validatorIndexes
  ) external override whenNotPaused {
    uint256 toMint;
    uint256 maxValidatorIndex = activatedValidators *
      1e4 +
      effectiveValidators() *
  ...
  function registerValidator(
    ...
    require(
      ...
      'Pool: invalid withdrawal credentials'
    );
