...
interface IStakedLyxToken is ILSP7DigitalAsset {
  ...
  event UnstakeMatched(uint256 amountMatched, uint256 newTotalPendingUnstake);

  event UnstakeReady(uint256 validatorsToExit);

  event UnstakeProcessed(uint256 unstakeAmount, uint256 newTotalPendingUnstake);
