...
// errors
import {LSP7AmountExceedsAuthorizedAmount, LSP7CannotSendToSelf, LSP7InvalidTransferBatch, LSP7CannotUseAddressZeroAsOperator, LSP7TokenOwnerCannotBeOperator, LSP7CannotSendWithAddressZero, LSP7AmountExceedsBalance, LSP7NotifyTokenReceiverContractMissingLSP1Interface, LSP7NotifyTokenReceiverIsEOA} from '@lukso/lsp-smart-contracts/contracts/LSP7DigitalAsset/LSP7Errors.sol';
  ...
  ReentrancyGuardUpgradeable
{
  ...
  function supportsInterface(
    bytes4 interfaceId
  ) public view virtual override(AccessControlUpgradeable, IERC165, ERC725YCore) returns (bool) {
  ...
  function isUnstakeRequestClaimable(uint256 index) public view override returns (bool) {
    ...
    if (index == unstakeRequestCurrentIndex && _request.amount != _request.amountFilled)
      return false;
  ...
  function matchUnstake(uint256 amount) external override returns (uint256) {
    ...
    if (totalPendingUnstake == 0) {
      ...
      _unstakeRequests[unstakeRequestCount].amountFilled = _unstakeRequests[unstakeRequestCount]
        .amount;
    } else {
  ...
  function setUnstakeProcessing() external override {
    ...
    require(
      totalPendingUnstake >= VALIDATOR_TOTAL_DEPOSIT,
      'StakedLyxToken: insufficient pending unstake'
    );

    unstakeProcessing = true;
    emit UnstakeReady(
      (totalPendingUnstake - (totalPendingUnstake % VALIDATOR_TOTAL_DEPOSIT)) /
  ...
  function claimUnstake(
    address account,
    uint256[] calldata unstakeRequestIndexes
    ...
    for (uint256 i = 0; i < unstakeRequestIndexes.length; i++) {
      ...
      require(
        isUnstakeRequestClaimable(unstakeRequestIndex),
  ...
  function _transfer(
    ...
    require(
      block.number > rewards.lastUpdateBlockNumber(),
      'StakedLyxToken: cannot transfer during rewards update'
    );
    ...
    (bool senderRewardsDisabled, bool recipientRewardsDisabled) = rewards.updateRewardCheckpoints(
      from,
      to
    );
    if (
      (senderRewardsDisabled || recipientRewardsDisabled) &&
      !(senderRewardsDisabled && recipientRewardsDisabled)
    ) {
