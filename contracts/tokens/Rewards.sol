...
contract Rewards is IRewards, OwnablePausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function updateRewardCheckpoint(
    address account
  ) public override returns (bool accRewardsDisabled) {
  ...
  function _updateRewardCheckpoint(address account, uint128 newRewardPerToken) internal {
    ...
    if (stakedLyxAmount == 0) {
      checkpoints[account] = Checkpoint({reward: cp.reward, rewardPerToken: newRewardPerToken});
    } else {
   ...
   */
  function updateRewardCheckpoints(
    address account1,
    address account2
  ) public override returns (bool rewardsDisabled1, bool rewardsDisabled2) {
  ...
  function updateTotalRewards(uint256 newTotalRewards) external override {
    ...
    uint256 newRewardPerToken = prevRewardPerToken +
      ((periodRewards - protocolReward) * 1e18) /
      stakedLyxToken.totalDeposits();
    ...
    if (_protocolFeeRecipient == address(0) && protocolReward > 0) {
      ...
      checkpoints[_protocolFeeRecipient] = Checkpoint({
        reward: (_balanceOf(_protocolFeeRecipient, newRewardPerToken) + protocolReward)
          .toUint128(),
