...
async function main() {
  ...
  let shouldUpdateEnv = process.env.UPDATE_ENV === "true";
  if (shouldUpdateEnv) {
    const envFilePath = path.join(__dirname, "../../..", "leequid-back", ".env");
    let envContent = fs.existsSync(envFilePath) ? fs.readFileSync(envFilePath, "utf8") : "";

    let envObject = dotenv.parse(Buffer.from(envContent));
    envObject = {
      ...envObject,
      deposit: mockBeacon.address,
      rewards: rewards.address,
      ...
      swap_pool_v1: "0xbeebdeaddeaddeaddeaddeaddeaddeaddeadbeef",
    };
    ...
    envContent = stringifyEnv(envObject);
